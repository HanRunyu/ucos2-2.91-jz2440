...
static noinline char *put_dec(char *buf, U32 num)
{
	while (1) {
		unsigned rem;
		if (num < 100000)
...
static char *number(char *buf, char *end, U32 num,
...
static int vsnprintf_internal(char *buf, U32 size, const char *fmt,
			      va_list args)
{
	U32 num;
	int base;
...
int vsnprintf(char *buf, U32 size, const char *fmt,
			      va_list args)
{
...
{
	return vsnprintf_internal(buf, CONFIG_SYS_PBSIZE, fmt, args);
}
